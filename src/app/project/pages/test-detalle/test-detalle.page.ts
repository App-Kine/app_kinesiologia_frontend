...
export class TestDetallePage implements OnInit {
  ...
  async confirmarQuitar(pregunta: any): Promise<void> {
    const alert = await this.alertCtrl.create({
      ...
      message: `
        <p>Se quitará la pregunta de este test.</p>
        <p>Si la pregunta no está en otros tests y nunca fue respondida, se eliminará del banco automáticamente.</p>
        <p><strong>Enunciado:</strong><br>
        <em>${this.escapar(pregunta.enunciado).substring(0, 200)}${pregunta.enunciado.length > 200 ? '...' : ''}</em></p>
      `,
      buttons: [
