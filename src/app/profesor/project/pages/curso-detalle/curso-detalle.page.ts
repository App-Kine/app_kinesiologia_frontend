...
export class CursoDetallePage implements OnInit {
  ...
  async confirmarEliminarCurso(): Promise<void> {
    ...
    const alert = await this.alertCtrl.create({
      ...
      message: `El curso "${this.curso.nombre}" se marcará como inactivo. Las aplicaciones y evaluaciones se conservan para histórico.`,
      buttons: [
  ...
  async confirmarQuitarAplicacion(a: any): Promise<void> {
    const alert = await this.alertCtrl.create({
      ...
      message: `Se eliminará la aplicación de "${a.test_nombre}" en este curso. Si hay evaluaciones de estudiantes registradas, no se podrá eliminar — usa desactivar en su lugar.`,
      buttons: [
        { text: 'Cancelar', role: 'cancel' },
        {
          text: 'Quitar',
