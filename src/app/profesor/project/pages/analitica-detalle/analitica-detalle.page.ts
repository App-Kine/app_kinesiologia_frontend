...
@Component({
  ...
  imports: [
    ...
    IonText, IonSegment, IonSegmentButton, SafeHtmlPipe,
  ],
})
export class AnaliticaDetallePage implements OnInit {
  ...
  /**
   * Vista por estudiante identificado, materializada UNA vez tras cargar los
   * datos (ver `recargar`). Antes era un getter que reconstruía el Record y
   * ordenaba en CADA change detection; ahora se calcula solo cuando cambian
   * los datos cargados, evitando trabajo repetido.
   */
  tiemposPorEstudiante: Array<{
  ...
  /**
   * ngOnInit captura el `aplicacionId` de la URL una sola vez (no cambia
   * durante la vida de la página). La carga de datos en sí se hace en
   * ionViewWillEnter para que refresque al volver de subnavegaciones.
   */
  ngOnInit(): void {
  ...
  async recargar(): Promise<void> {
    ...
    try {
      ...
      this.tiemposPorEstudiante = this.calcularTiemposPorEstudiante();
    } catch (err: any) {
      this.error = (err && err.message) || 'No se pudo cargar la analítica.';
      this.data = null;
      this.tiemposPorEstudiante = [];
    } finally {
      this.cargando = false;
    }
  }
   ...
   */
  private calcularTiemposPorEstudiante(): Array<{
    evaluacion_id: number;
    correo: string;
    pregs: Array<{ orden: number; tiempo: number; resultado: string | null }>;
  }> {
